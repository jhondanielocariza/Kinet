...
console.log(chalk.bold.hex("#FF9000").bold("[ killua 2.0.2 ] » ") + chalk.bold.hex("#FF9000").bold("Initializing variables..."));
...
loginApiData.setOptions(global.config.FCAOption)
        ...
        logger(`killua S. 2.0.2`, '[ killua ]');
        logger('Hey, thank you for using this Bot', '[ killua ]');
        logger("Fixed by killua", '[ killua ]');
...
cron.schedule('0 0 12 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("maganda hapon sa lahat", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
...
cron.schedule('0 0 18 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("magandang gabe, tulog na kayo maaga iba na gusto nun", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
...
He will wipe away every tear from their eyes, and death shall be no more, neither shall there be mourning, nor crying, nor pain anymore, for the former things have passed away. And He who was seated on the throne said, Behold, I am making all things new.`];
  ...
  loginApiData.getThreadList(50, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage(`Random bible verse every 3 hours:\n\n ${juswa1}\nAmen`, now.threadID) : '');
  });
  }, {
  scheduled: true,
  timezone: "Asia/Manila"
